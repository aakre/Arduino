...
void MatrixMath::Diag(float* d, int n, float *C) {
  for (int i=0; i<n; i++) {
    for (int j=0; j<n; j++) {
      if (i==j) {
        C[n*i+j] = d[i];
...
void MatrixMath::Zeros(float *C, int row, int col){
  for (int i=0; i<row; i++) {
    for (int j=0; j<col; j++) {
      C[i*col+j] = 0;
    }
...
void MatrixMath::Eye(float* I, int n) {
  // Or: create a new vector with ones and call diag
  for (int i=0; i<n; i++){
    for (int j=0; j<n; j++) {
      if (i==j) {
        I[n*i+j] = 1;
